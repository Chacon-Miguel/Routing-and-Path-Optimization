...
class DirectedRoad():
    ...
    def get_travel_time(self, has_traffic=False):
        ...
        """
        if has_traffic:
            return self.travel_time * self.get_traffic_multiplier()
    ...
    def __str__(self):
        ...
        """
        src_node = self.get_source_node().get_name()
        dest_node = self.get_destination_node().get_name()
        travel_time = str(self.get_travel_time())
        road_type = self.get_road_type()
        traffic_multiplier = str(self.get_traffic_multiplier())
        return src_node + " -> " + dest_node + " takes " + travel_time + \
            " minute(s) via " + road_type + " road with traffic multiplier " + traffic_multiplier
...
class RoadMap():
    ...
    def get_reachable_roads_from_node(self, node, restricted_roads):
        ...
        """
        if self.contains_node(node):
            possible_roads = self.nodes_to_roads[node].copy()
            roads = []
